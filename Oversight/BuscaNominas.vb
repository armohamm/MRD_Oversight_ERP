Public Class BuscaNominas
    ...
    Private Sub BuscaNominas_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing

        executeSQLCommand(0, "INSERT IGNORE INTO logs VALUES (" & getMySQLDate() & ", '" & getAppTime() & "', '" & susername & "', " & susersession & ", '" & suserip & "', '" & susermachinename & "', 'Cerró la ventana de Buscar Nomina con la Nomina " & ipayrollid & " seleccionada', 'OK')")

    End Sub


    Private Sub BuscaNominas_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub BuscaNominas_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        queryBusqueda = "" & _
        "SELECT " & _
        "pr.ipayrollid, STR_TO_DATE(CONCAT(pr.ipayrolldate, ' ', pr.spayrolltime), '%Y%c%d %T') AS 'Fecha Nomina', " & _
        ...
        "FORMAT(IF(SUM((prpe.ddaysworked * prpe.ddaysalary) + (prpe.dextrahours * prpe.dextrahoursalary) + prpe.dextraincomeamount - prpe.ddiscountamount) IS NULL, 0, SUM((prpe.ddaysworked * prpe.ddaysalary) + (prpe.dextrahours * prpe.dextrahoursalary) + prpe.dextraincomeamount)), 2) AS 'Total Nomina Sin Descuentos', " & _
        ...
        "GROUP BY 1 " & _
        "ORDER BY 7 DESC, 8 DESC, 5 ASC, 6 ASC "

        txtBuscar.Enabled = True

        setDataGridView(dgvNominas, queryBusqueda, True)

        dgvNominas.Columns(0).Visible = False

        executeSQLCommand(0, "INSERT IGNORE INTO logs VALUES (" & getMySQLDate() & ", '" & getAppTime() & "', '" & susername & "', " & susersession & ", '" & suserip & "', '" & susermachinename & "', 'Abrió la Ventana de Buscar Nominas', 'OK')")
    ...
    Private Sub dgvNominas_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvNominas.CellClick
        ...
        Try
            ...
            If dgvNominas.CurrentRow Is Nothing Then
                Exit Sub
            End If

            ipayrollid = CInt(dgvNominas.Rows(e.RowIndex).Cells(0).Value)
    ...
    Private Sub dgvNominas_CellContentClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvNominas.CellContentClick
    ...
    Private Sub dgvNominas_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvNominas.SelectionChanged
        ...
        Try
            ...
            ipayrollid = CInt(dgvNominas.CurrentRow.Cells(0).Value)
    ...
    Private Sub dgvNominas_CellContentDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvNominas.CellContentDoubleClick
        ...
        If dgvNominas.SelectedRows.Count = 1 Then
            ...
            If an.DialogResult = Windows.Forms.DialogResult.OK Then

                Me.ipayrollid = an.ipayrollid
    ...
    Private Sub dgvNominas_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvNominas.CellDoubleClick
    ...
    Private Sub btnExportar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExportar.Click
        ...
        Try
            ...
            msSaveFileDialog.FileName = "Nominas " & fecha
            ...
            If msSaveFileDialog.ShowDialog() = DialogResult.OK Then
                ...
                If resultado = True Then
                    MsgBox("Nominas Exportadas Correctamente!" & Chr(13) & "El archivo se abrirá al dar click en OK", MsgBoxStyle.OkOnly, "Exportación Completada")
                    System.Diagnostics.Process.Start(msSaveFileDialog.FileName)
                Else
                    MsgBox("No se ha podido exportar las Nominas. Intente nuevamente.", MsgBoxStyle.OkOnly, "Error al exportar las Nominas")
                End If
    ...
    Private Function ExportToExcel(ByVal pth As String) As Boolean
        ...
        Try
            ...
            fs.WriteLine("   <Column ss:AutoFitWidth=""0"" ss:Width=""76.5""/>")
            fs.WriteLine("   <Column ss:AutoFitWidth=""0"" ss:Width=""64.5""/>")
            fs.WriteLine("   <Column ss:Width=""84""/>")
            fs.WriteLine("   <Column ss:AutoFitWidth=""0"" ss:Width=""193.5""/>")
            fs.WriteLine("   <Column ss:AutoFitWidth=""0"" ss:Width=""54""/>")
            fs.WriteLine("   <Column ss:AutoFitWidth=""0"" ss:Width=""54.75""/>")
            fs.WriteLine("   <Column ss:Width=""145.5""/>")
            fs.WriteLine("   <Column ss:AutoFitWidth=""0"" ss:Width=""85.5""/>")
            fs.WriteLine("   <Column ss:Width=""78""/>")

            fs.WriteLine("   <Row ss:AutoFitHeight=""0"">")
            fs.WriteLine("  <Cell ss:MergeAcross=""9"" ss:StyleID=""1""><Data ss:Type=""String"">NOMINAS</Data></Cell>")
            fs.WriteLine("   </Row>")
