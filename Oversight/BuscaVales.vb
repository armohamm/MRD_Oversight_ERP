Public Class BuscaVales
    ...
    Private Sub setControlsByPermissions(ByVal windowname As String, ByVal username As String)
        ...
        If viewPermission = False Then
            ...
            executeSQLCommand(0, "INSERT IGNORE INTO logs VALUES (" & fecha & ", '" & hora & "', '" & susername & "', " & susersession & ", '" & suserip & "', '" & susermachinename & "', 'Acceso denegado a la ventana de Buscar Vales', 'OK')")
    ...
    Private Sub BuscaVales_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing

        executeSQLCommand(0, "INSERT IGNORE INTO logs VALUES (" & getMySQLDate() & ", '" & getAppTime() & "', '" & susername & "', " & susersession & ", '" & suserip & "', '" & susermachinename & "', 'Cerró la ventana de Buscar Vales con el vale " & igasvoucherid & "', 'OK')")

    End Sub


    Private Sub BuscaVales_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub BuscaVales_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        queryBusqueda = "" & _
        "SELECT g.igasvoucherid, g.iassetid, STR_TO_DATE(CONCAT(g.igasdate, ' ', g.sgastime), '%Y%c%d %T') AS 'Fecha', g.igasvoucherfolio AS 'Folio', a.sassetdescription AS 'Auto', g.scarmileageatrequest AS 'Kms al momento', " & _
        "g.scarorigindestination AS 'Origen Destino', g.svouchercomment AS 'Comentarios', g.ipeopleid, p.speoplefullname AS 'Persona', " & _
        "FORMAT(g.dlitersdispensed, 3) AS 'Litros', gt.sgastype AS 'Tipo', FORMAT(g.dgasvoucheramount, 2) AS 'Monto del Vale' " & _
        ...
        "OR g.igasvoucherfolio LIKE '%" & querystring & "%' " & _
        "ORDER BY 3 DESC "
        txtBuscar.Enabled = True

        setDataGridView(dgvVales, queryBusqueda, True)
        ...
        dgvVales.Columns(2).Width = 130
        dgvVales.Columns(3).Width = 50
        dgvVales.Columns(4).Width = 200
        dgvVales.Columns(5).Width = 50
        dgvVales.Columns(6).Width = 100
        dgvVales.Columns(7).Width = 100
        dgvVales.Columns(8).Width = 30
        dgvVales.Columns(9).Width = 150
        dgvVales.Columns(10).Width = 60
        dgvVales.Columns(11).Width = 50
        dgvVales.Columns(12).Width = 60

        dgvVales.Select()

        executeSQLCommand(0, "INSERT IGNORE INTO logs VALUES (" & getMySQLDate() & ", '" & getAppTime() & "', '" & susername & "', " & susersession & ", '" & suserip & "', '" & susermachinename & "', 'Abrió la Ventana de Buscar Vales', 'OK')")
    ...
    Private Sub dgvVales_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvVales.CellClick

        Try

            If dgvVales.CurrentRow Is Nothing Then
                Exit Sub
            End If

            igasvoucherid = CInt(dgvVales.Rows(e.RowIndex).Cells(0).Value)
            iassetid = CInt(dgvVales.Rows(e.RowIndex).Cells(1).Value)
            ipeopleid = CInt(dgvVales.Rows(e.RowIndex).Cells(8).Value)
    ...
    Private Sub dgvVales_CellContentClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvVales.CellContentClick
    ...
    Private Sub dgvVales_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvVales.SelectionChanged
        ...
        Try
            ...
            igasvoucherid = CInt(dgvVales.CurrentRow.Cells(0).Value)
            iassetid = CInt(dgvVales.CurrentRow.Cells(1).Value)
            ipeopleid = CInt(dgvVales.CurrentRow.Cells(8).Value)
    ...
    Private Sub dgvVales_CellContentDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvVales.CellContentDoubleClick
        ...
        If dgvVales.SelectedRows.Count = 1 Then
            ...
            av.igasvoucherid = igasvoucherid
            av.iassetid = iassetid
            av.ipeopleid = ipeopleid
    ...
    Private Sub dgvVales_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvVales.CellDoubleClick
    ...
    Private Sub btnCrear_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCrear.Click
        ...
        If av.DialogResult = Windows.Forms.DialogResult.OK Then
            ...
            Me.igasvoucherid = av.igasvoucherid
            Me.iassetid = av.iassetid
            Me.ipeopleid = av.ipeopleid

            If av.wasCreated = True Then
                Me.wasCreated = True
            End If
    ...
    Private Sub btnExportar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExportar.Click
        ...
        Try
            ...
            msSaveFileDialog.FileName = "Vales de Gasolina " & fecha
            ...
            If msSaveFileDialog.ShowDialog() = DialogResult.OK Then
                ...
                If resultado = True Then
                    MsgBox("Vales Exportados Correctamente!" & Chr(13) & "El archivo se abrirá al dar click en OK", MsgBoxStyle.OkOnly, "Exportación Completada")
                    System.Diagnostics.Process.Start(msSaveFileDialog.FileName)
                Else
                    MsgBox("No se ha podido exportar los Vales. Intente nuevamente.", MsgBoxStyle.OkOnly, "Error al exportar los Vales")
                End If
    ...
    Private Function ExportToExcel(ByVal pth As String) As Boolean
        ...
        Try
            ...
            fs.WriteLine("   <Column ss:AutoFitWidth=""0"" ss:Width=""60.75""/>")
            fs.WriteLine("   <Column ss:Width=""204""/>")
            fs.WriteLine("   <Column ss:AutoFitWidth=""0"" ss:Width=""58.5""/>")
            fs.WriteLine("   <Column ss:AutoFitWidth=""0"" ss:Width=""165.75""/>")
            fs.WriteLine("   <Column ss:AutoFitWidth=""0"" ss:Width=""125.25"" ss:Span=""1""/>")
            fs.WriteLine("   <Column ss:Width=""39.75""/>")
            fs.WriteLine("   <Column ss:Width=""48""/>")
            fs.WriteLine("   <Column ss:Width=""85.5""/>")

            fs.WriteLine("   <Row ss:AutoFitHeight=""0"">")
            fs.WriteLine("  <Cell ss:MergeAcross=""9"" ss:StyleID=""1""><Data ss:Type=""String"">VALES DE GASOLINA</Data></Cell>")
            fs.WriteLine("   </Row>")
            ...
            For Each col As DataGridViewColumn In dgvVales.Columns
                If col.Visible Then
            ...
            For Each row As DataGridViewRow In dgvVales.Rows

                If dgvVales.AllowUserToAddRows = True And row.Index = dgvVales.Rows.Count - 1 Then
                    Exit For
                End If
