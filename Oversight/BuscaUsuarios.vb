Public Class BuscaUsuarios
    ...
    Public sselectedpeoplename As String = ""

    Public isEdit As Boolean = False
    Public wasCreated As Boolean = False

    Private openPermission As Boolean = False
    Private viewOnline As Boolean = False
    Private viewActive As Boolean = False
    Private viewLastModDate As Boolean = False
    Private viewLastModUser As Boolean = False
    ...
    Private Sub setControlsByPermissions(ByVal windowname As String, ByVal username As String)
        ...
        For j = 0 To dsPermissions.Tables(0).Rows.Count - 1
            ...
            Try
                ...
                If permission = "Ver Activo" Then
                    viewActive = True
                End If

                If permission = "Ver Online" Then
                    viewOnline = True
                End If

                If permission = "Ver Fecha Ultima Mod" Then
                    viewLastModDate = True
                End If

                If permission = "Ver Usuario Ultima Mod" Then
                    viewLastModUser = True
                End If
        ...
        If viewPermission = False Then
            ...
            executeSQLCommand(0, "INSERT IGNORE INTO logs VALUES (" & fecha & ", '" & hora & "', '" & susername & "', " & susersession & ", '" & suserip & "', '" & susermachinename & "', 'Acceso denegado a la ventana de Buscar Usuarios', 'OK')")
    ...
    Private Sub BuscaUsuarios_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing

        executeSQLCommand(0, "INSERT IGNORE INTO logs VALUES (" & getMySQLDate() & ", '" & getAppTime() & "', '" & susername & "', " & susersession & ", '" & suserip & "', '" & susermachinename & "', 'Cerró la ventana de Buscar Usuario con el Usuario " & sselectedusername & " : " & sselectedpeoplename & " seleccionado', 'OK')")

    End Sub


    Private Sub BuscaUsuarios_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub BuscaUsuarios_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        queryBusqueda = "" & _
        "SELECT u.susername AS 'Nombre Usuario', p.speoplefullname AS 'Nombre Completo', IF(u.bactive = 1, 'Activo', 'Inactivo') AS 'Activo / Inactivo', " & _
        "IF(u.bonline = 1, 'Online', 'Offline') AS 'Online / Offline', STR_TO_DATE(CONCAT(u.iupdatedate, ' ', u.supdatetime), '%Y%c%d %T') AS 'Fecha Ultima Mod', " & _
        "u.supdateusername AS 'Usuario Ultima Mod' " & _
        "FROM users u " & _
        "JOIN people p ON u.ipeopleid = p.ipeopleid " & _
        "WHERE u.susername LIKE '%" & querystring & "%' OR u.supdateusername LIKE '%" & querystring & "%' OR p.speoplefullname LIKE '%" & querystring & "%' " & _
        "OR IF(u.bactive = 1, 'Activo', 'Inactivo') LIKE '%" & querystring & "%' OR IF(u.bonline = 1, 'Online', 'Offline') LIKE '%" & querystring & "%' " & _
        "ORDER BY 1 "

        txtBuscar.Enabled = True

        setDataGridView(dgvUsuarios, queryBusqueda, True)

        If viewActive = False Then
            dgvUsuarios.Columns(2).Visible = False
        End If

        If viewOnline = False Then
            dgvUsuarios.Columns(3).Visible = False
        End If

        If viewLastModDate = False Then
            dgvUsuarios.Columns(4).Visible = False
        End If

        If viewLastModUser = False Then
        ...
        dgvUsuarios.Columns(4).Width = 70
        dgvUsuarios.Columns(5).Width = 100

        executeSQLCommand(0, "INSERT IGNORE INTO logs VALUES (" & getMySQLDate() & ", '" & getAppTime() & "', '" & susername & "', " & susersession & ", '" & suserip & "', '" & susermachinename & "', 'Abrió la Ventana de Buscar Usuarios', 'OK')")
    ...
    Private Sub dgvUsuarios_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvUsuarios.CellClick
        ...
        Try
            ...
            sselectedusername = dgvUsuarios.Rows(e.RowIndex).Cells(0).Value
            sselectedpeoplename = dgvUsuarios.Rows(e.RowIndex).Cells(1).Value
    ...
    Private Sub dgvUsuarios_CellContentClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvUsuarios.CellContentClick
    ...
    Private Sub dgvUsuarios_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvUsuarios.SelectionChanged
        ...
        Try
            ...
            sselectedusername = dgvUsuarios.CurrentRow.Cells(0).Value
            sselectedpeoplename = dgvUsuarios.CurrentRow.Cells(1).Value
    ...
    Private Sub dgvUsuarios_CellContentDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvUsuarios.CellContentDoubleClick
        ...
        If dgvUsuarios.SelectedRows.Count = 1 Then
            ...
            au.sselectedusername = sselectedusername
