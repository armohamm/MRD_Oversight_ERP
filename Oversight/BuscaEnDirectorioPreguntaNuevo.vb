Public Class BuscaEnDirectorioPreguntaNuevo
    ...
    Public iselectedoption As Integer = 0

    Private openPermission As Boolean = False

    Private newPeoplePermission As Boolean = False
    Private newSupplierPermission As Boolean = False
    ...
    Private Sub setControlsByPermissions(ByVal windowname As String, ByVal username As String)
        ...
        For j = 0 To dsPermissions.Tables(0).Rows.Count - 1
            ...
            Try
                ...
                If permission = "Crear Personas" Then
                    newPeoplePermission = True
                End If

                If permission = "Crear Proveedores" Then
                    newSupplierPermission = True
                End If
        ...
        If viewPermission = False Then
            ...
            executeSQLCommand(0, "INSERT IGNORE INTO logs VALUES (" & fecha & ", '" & hora & "', '" & susername & "', " & susersession & ", '" & suserip & "', '" & susermachinename & "', 'Acceso denegado a la ventana de Escoger Tipo de Persona/Proveedor Nuevo', 'OK')")
    ...
    Private Sub BuscaEnDirectorioPreguntaNuevo_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing

        If iselectedoption = 1 Then
            executeSQLCommand(0, "INSERT IGNORE INTO logs VALUES (" & getMySQLDate() & ", '" & getAppTime() & "', '" & susername & "', " & susersession & ", '" & suserip & "', '" & susermachinename & "', 'Cerró la ventana de Escoger Tipo de Persona/Proveedor Nuevo con el tipo Persona seleccionado', 'OK')")
        ElseIf iselectedoption = 2 Then
            executeSQLCommand(0, "INSERT IGNORE INTO logs VALUES (" & getMySQLDate() & ", '" & getAppTime() & "', '" & susername & "', " & susersession & ", '" & suserip & "', '" & susermachinename & "', 'Cerró la ventana de Escoger Tipo de Persona/Proveedor Nuevo con el tipo Proveedor seleccionado', 'OK')")
        End If

    End Sub


    Private Sub BuscaEnDirectorioPreguntaNuevo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub BuscaEnDirectorioPreguntaNuevo_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        executeSQLCommand(0, "INSERT IGNORE INTO logs VALUES (" & getMySQLDate() & ", '" & getAppTime() & "', '" & susername & "', " & susersession & ", '" & suserip & "', '" & susermachinename & "', 'Abrió la Ventana de Escoger Tipo de Persona/Proveedor Nuevo', 'OK')")

        If newSupplierPermission = False Then
            rbProveedor.Visible = False
        End If

        If newPeoplePermission = False Then
            rbPersona.Visible = False
        Else
            rbPersona.Select()
            rbPersona.Focus()
        End If
    ...
    Private Sub btnGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnActualizar.Click

        iselectedoption = 0

        If rbPersona.Checked = True Then
            iselectedoption = 1
        End If
